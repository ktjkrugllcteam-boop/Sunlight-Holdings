...
/*
 * Virtual Tour Component - Dynamic Version
 * Supports new JSON structure (Multi-language) with auto-scaling map
 */
...
export default function VirtualTour({ 
  ...
  const getLocalized = useCallback((content: any) => {
  ...
  const mapBounds = useMemo(() => {
    ...
    const maxX = Math.max(...rooms.map(r => r.mapX || 0), 100) + 50; 
    const maxY = Math.max(...rooms.map(r => r.mapY || 0), 100) + 50;
    return { w: maxX, h: maxY };
  }, [rooms]);
    ...
    <AnimatePresence>
      ...
      >
        ...
        <div className="relative w-full h-full overflow-hidden">
          <AnimatePresence mode="wait">
            ...
            >
              {currentImageUrl ? (
                <img
                  src={currentImageUrl}
                  // 2️⃣ Localize Alt Text
                  alt={getLocalized(currentRoom.roomName)}
                  className="w-full h-full object-cover"
                />
              ) : (
                <div className="w-full h-full flex items-center justify-center text-white/40">
          ...
          <div className="absolute top-0 left-0 right-0 p-6 flex items-center justify-between z-20">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h2 className="font-serif text-2xl text-white">
                  {/* 3️⃣ Localize Title */}
                  {getLocalized(currentRoom.roomName)}
                </h2>
                <p className="text-white/60 text-sm">
                  {/* 4️⃣ Localize Description */}
                  {getLocalized(currentRoom.description)}
                </p>
              </div>
            </div>
          ...
          {/* Room Navigation Hotspots (Middle Buttons) */}
          <div className="absolute bottom-32 left-1/2 -translate-x-1/2 flex items-center gap-4 z-20 flex-wrap justify-center">
            {connectedRooms.map((room) => (
              <motion.button
                key={room.roomId}
                onClick={() => navigateToRoom(room.roomId)}
              ...
              >
                <Eye size={18} className="text-[#2962ff]" />
                <span className="text-white font-display text-sm tracking-wider uppercase">
                  {/* 5️⃣ Localize Button Text */}
                  {getLocalized(room.roomName)}
                </span>
                <ArrowRight size={16} className="text-white/50 group-hover:text-[#d4af37] group-hover:translate-x-1 transition-all" />
              </motion.button>
            ))}
                ...
                <div className="relative aspect-[4/3] bg-[#0d1220] border border-[#2962ff]/10">
                  ...
                  {rooms.map((room) => (
                    <motion.button
                      ...
                      whileHover={{ scale: 1.3 }}
                      // 7️⃣ Localize Map Tooltip
                      title={getLocalized(room.roomName)}
                    />
                  ))}
