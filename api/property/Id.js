...
async function handler(req, res) {
  ...
  try {
    ...
    // ================= SECURITY GATE =================
    // Only GET is public. PUT and DELETE require admin verification.
    if (method !== "GET") {
      try {
    ...
    if (method === "GET") {
      const [rows] = await connection.query(
        `
        SELECT 
          p.Pid, p.projectName, p.location, p.description, p.thumbImage, p.price,
          r.RoomId, r.RoomName, r.Description AS roomDescription, r.MapX, r.MapY,
          pi.ImageID, pi.ImageUrl,
          rc.ConnectionID, rc.FromRoomId, rc.ToRoomId
        FROM Properties p
        LEFT JOIN Rooms r ON r.Pid = p.Pid
        LEFT JOIN PropertyImages pi ON pi.RoomId = r.RoomId
        LEFT JOIN RoomConnections rc ON rc.FromRoomId = r.RoomId OR rc.ToRoomId = r.RoomId
        WHERE p.Pid = ?
        `,
        [Id]
      );
      ...
      rows.forEach((row) => {
        ...
        if (row.ImageID && !property.rooms[row.RoomId].images.some(img => img.imageID === row.ImageID)) {
          property.rooms[row.RoomId].images.push({ imageID: row.ImageID, url: row.ImageUrl });
        }

        if (row.ConnectionID && !property.rooms[row.RoomId].connections.some(c => c.connectionID === row.ConnectionID)) {
          property.rooms[row.RoomId].connections.push({
            connectionID: row.ConnectionID,
            from: row.FromRoomId,
            to: row.ToRoomId,
    ...
    // ================= PUT METHOD =================
    if (method === "PUT") {
  const { projectName, location, description, thumbImage, price } = req.body;

  const fields = [];
  const values = [];

  const jsonColumns = ["projectName", "location", "description", "price"];

  const updates = { projectName, location, description, thumbImage, price };
  ...
  return res.status(200).json({ message: "Property updated successfully" });
    ...
    if (method === "DELETE") {
      await connection.beginTransaction();
      try {
        const [rooms] = await connection.execute(`SELECT RoomId FROM Rooms WHERE Pid = ?`, [Id]);
        const roomIds = rooms.map((r) => r.RoomId);
        ...
        if (roomIds.length > 0) {
          ...
          // Delete Connections
          await connection.execute(
            `DELETE FROM RoomConnections WHERE FromRoomId IN (${placeholders}) OR ToRoomId IN (${placeholders})`,
            [...roomIds, ...roomIds]
          );
          // Delete Images
          await connection.execute(`DELETE FROM PropertyImages WHERE RoomId IN (${placeholders})`, roomIds);
          // Delete Rooms
          await connection.execute(`DELETE FROM Rooms WHERE Pid = ?`, [Id]);
        }

        const [resProp] = await connection.execute(`DELETE FROM Properties WHERE Pid = ?`, [Id]);
        if (resProp.affectedRows === 0) {
          await connection.rollback();
          return res.status(404).json({ message: "Property not found" });
        }

        await connection.commit();
        return res.status(200).json({ message: "Property and associated data deleted" });
      } catch (err) {
        await connection.rollback();
        throw err;
      }
